...
print_config = config_parser.read(os.path.join(
...
def processed_folder_check(path):
    '''This functions checks if processed folder exists to save output
    if not it makes a new processed folder'''
    try:
        for folder in os.listdir(config.path):
...
def get_gene_symbols_from_moka(run_df):
    ...
    try:
        ...
        for patient_pan_number in unique_pan_numbers_to_find:
            ...
            check_dmlpa_list = ['","'.join(map(str, row))
                                for row in check_dmlpa_sql]
            ...
            # Check if the first element of the list matches
               ...
               # exception_error_msg = "blank"
                ...
                gene_list = ['","'.join(map(str, row))
                             for row in get_genes_sql]
...
def filter_results(run_df, path_to_runfolder, pan_numbers_genes_dic):
    ...
    filters_used_df = pd.DataFrame(
        columns=["Patient ID", "Pan number", "Genes in panel"])
    ...
    for column in patient_results_passed_df:
        ...
        try:
            ...
            sample_info = patient_results_passed_df[column].drop(
                patient_results_passed_df[column].index[7:])
            ...
            patient_results_passed_drop_df = patient_results_passed_df[column].drop(
                patient_results_passed_df[column].index[0:7])
            # Concat the probes dataframe to the patients results
            per_patient_df = pd.concat(
                [probes_df, patient_results_passed_drop_df], axis=1)
            ...
            if pan_numbers_genes_dic[patient_pan_number] == 1:
                ...
                filters_used_df_patient = pd.DataFrame({'Patient ID': column,
                                                       ...
                                                       index=[0, 1, 2])
                filters_used_df = pd.concat(
                    [filters_used_df, filters_used_df_patient], axis=0, ignore_index=True)
                ...
                patient_gene_filtered_df = per_patient_df.loc[per_patient_df['Gene'].isin(
                    pan_numbers_genes_dic[patient_pan_number])]
                # Get the control probes for this patient
                patient_control_df = per_patient_df.loc[per_patient_df['Probe type'].str.match(
                    "CTRL")]
                # Concat the gene filtered & control probes
                patient_gene_filtered_control_df = pd.concat(
                    [patient_gene_filtered_df, patient_control_df])
                ...
                patient_gene_filtered_control_df[column] = patient_gene_filtered_control_df[column].astype(
                    float, errors="ignore").round(3)
                # Create a xlsx to write data too
                writer = pd.ExcelWriter(
                    path_to_runfolder + "/" + column + "_filtered_results.xlsx", engine="xlsxwriter")
                # Write each dataframe to a different worksheet.
                sample_info.to_excel(
                    writer, sheet_name='Sample info', index=False)
                patient_gene_filtered_control_df.to_excel(
                    writer, sheet_name='Gene filtered results', index=False)
                # Close the Pandas Excel writer and output the Excel file.
                writer.save()
                #  Make a df of the genes used for this sample
                filters_used_df_patient = pd.DataFrame({'Patient ID': column,
                                                       'Pan number': patient_pan_number,
                                                        'Genes in panel': [(" ".join((pan_numbers_genes_dic[patient_pan_number])))]}, index=[0, 1, 2])
...
def create_and_save_logfile(logfile, failed_df,  path_to_runfolder):
    ...
    if failed_df.empty:
        ...
        logfile_no_failed_samples = pd.DataFrame({'Patient ID': "No failed samples in run",
                                                 ...
                                                 index=[0, 1, 2])
        logfile = pd.concat(
            [logfile, logfile_no_failed_samples], axis=0, ignore_index=True)
    else:
        for column in failed_df:
            logfile_failed_samples = pd.DataFrame({'Patient ID': column,
                                                  ...
                                                  index=[0, 1, 2])
            logfile = pd.concat(
                [logfile, logfile_failed_samples], axis=0, ignore_index=True)
    # As this df was made in a loop, there are duplicate entries
    logfile_duplicates_removed = logfile.drop_duplicates()
    filepath_log = path_to_runfolder + \
        "/logfile/"+file.split(".")[0] + "_log.csv"
...
try:
    ...
    error_message = str(exception_error_msg) + \
        ". Error connecting to Moka. Contact bioinformatics team "
...
try:
    for file in os.listdir(config.path):
        ...
        if file.endswith(config.samplesheet_extension):
            ...
            if error_occurred == True:
                error_message = file + \
                    " sheet doesn't have the expected number of rows. Contact bioinformatics team "
                error_log(path_to_runfolder, error_message)
            if processed_status == 1:
                ...
                gene_list, exception_error_msg = get_gene_symbols_from_moka(
                    loaded_df)
                if exception_error_msg is not None:
                    error_log(path_to_runfolder,  exception_error_msg)
                else:
                    logfile, failed_samples, exception_error_msg = filter_results(
                        loaded_df, path_to_runfolder, gene_list)
                    # Create logfile per run
                    create_and_save_logfile(
                        logfile, failed_samples,  path_to_runfolder)
                    if exception_error_msg is not None:
                        error_log(path_to_runfolder,  exception_error_msg)
                    else:
    ...
    if count == 0:
        print("No files ending in " +
              config.samplesheet_extension + " in folder to process")
except Exception as exception_error_msg:
    error_occurred = True
    if error_occurred == True:
